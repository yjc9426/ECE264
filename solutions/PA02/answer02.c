...
char * strcat_ex(char * * dest, int * n, const char * src){
    if((*dest)==NULL){
        char * new_buf = (char *) malloc(1 + 2 * (strlen(src)));	
        *n = 1 + 2 * (strlen(src));	
        *dest = new_buf;
	**dest = '\0';
    }
    else if( (*n) < strlen(*dest)+strlen(src)+1){
        char * new_buf = (char *) malloc(1 + 2 * (strlen(*dest)+strlen(src)));
...
char * * explode(const char * str, const char * delims, int * arrLen){
    ...
    for(i=0;i<strlen(str);i++){
        if(strchr(delims, *(str+i))!=NULL){
            char * new_str = (char *)malloc(i-last+1);
            memcpy(new_str, str+last, i-last);
            *(new_str+i-last) = '\0';
	    last = i+1;
            strArr[arrInd++] = new_str;
        }
    }
    char * new_str = (char *)malloc(i-last+1);
    memcpy(new_str, str+last, i-last);
...
char * implode(char * * strArr, int len, const char * glue){
    int i = 0;
    int N = strlen(strArr[0])+1;
    char * ret = (char *)malloc(N);
    *(ret) = '\0';   
    for(i=0;i<len;i++){
        if(i!=0){
            ret = strcat_ex(&ret, &N, glue);
        }
        ret = strcat_ex(&ret, &N, strArr[i]);
    }
...
int cmpstr(const void * str1, const void * str2){
    int i = 0;
    do{
        if(*(int*)(str1+i) - *(int*)(str1+i)!=0){
            return *(int*)(str1+i) - *(int*)(str1+i);
        }
        i++;
...
void sortStringArray(char * * arrString, int len){
    qsort(arrString, len, sizeof(char *), cmpstring);
}
